...
def Po_bod(polomer, uhel, R):
    x = Po_s(polomer, R)*cos(Po_d(uhel))
    y = Po_s(polomer, R)*sin(Po_d(uhel))
    return x, y

# vypocet a vykresleni bodu
def vykresli_Po_bod(y, x, R):
    bod = Po_bod(y, x, R)
    setpos(bod[0], bod[1])
